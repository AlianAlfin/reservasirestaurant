...
@section('container')
    ...
    <header class="w-full container mx-auto lg:pt-[85px] pt-[74px]">
        <div class="flex flex-col items-center py-12">
            <a class="font-bold text-gray-800 uppercase hover:text-gray-700 text-5xl font-h1" href="#">
                Blog
            </a>
        </div>
    </header>
    ...
    <div class="container mx-auto flex flex-wrap py-6">
        ...
        <section class="w-full md:w-2/3 flex flex-col items-center px-3">
            @foreach ($category_blogs as $categoryBlog)
                <article class="flex flex-col shadow my-4">
                    <!-- Article Image -->
                    <a href="{{ route('category_blogs.show', $categoryBlog->slug) }}" class="hover:opacity-75">
                        <img src="{{ Storage::url($categoryBlog->image) }}" class="object-cover">
                    </a>
                    <div class="bg-white flex flex-col justify-start p-6">
                        {{-- <a href="#" class="text-blue-700 text-sm font-bold uppercase pb-4">Technology</a> --}}
                        <a href="{{ route('category_blogs.show', $categoryBlog->slug) }}"
                            class="text-3xl font-bold hover:text-gray-700 pb-4">{{ $categoryBlog->title }}</a>
                        <p href="{{ route('category_blogs.show', $categoryBlog->slug) }}" class="text-sm pb-3">
                            By <a href="#" class="font-semibold hover:text-gray-800">Admin</a>
                        </p>
                        <a href="{{ route('category_blogs.show', $categoryBlog->slug) }}"
                            class="pb-6">{{ $categoryBlog->preview }}</a>
                        <a href="{{ route('category_blogs.show', $categoryBlog->slug) }}" class="pb-6">Realese :
        ...
        <aside class="w-full md:w-1/3 flex flex-col items-center px-3">
            ...
            <div class="w-full bg-white shadow flex flex-col my-4 p-6">
                <p class="text-xl font-semibold pb-5">Galery</p>
